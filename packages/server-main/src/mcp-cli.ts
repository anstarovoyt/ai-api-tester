...
const formatBanner = (label: string) => colorize("bold", colorize("cyan", label));
...
const printResponse = (payload: unknown) => {
  const text = typeof payload === "string" ? payload : JSON.stringify(payload, null, 2);
  console.log(`\n${formatBanner("< RESPONSE")}\n${colorize("green", text)}`);
};

runtime.on("log", (entry: any) => {
...
const parseParams = (jsonText: string) => {
...
const sendRequest = (method: string, params: unknown) => {
  ...
  void runtime.sendRequest(payload).catch((err) => {
